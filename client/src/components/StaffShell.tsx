...
import {
  ArrowLeftRight, BadgePercent, BarChart3, ChevronDown, ChevronRight, ChevronsDownUp, ClipboardCheck, LayoutDashboard, LogOut, Menu, Package, ReceiptText, Settings2, ShoppingCart, Store, Undo2, UsersRound, UserCog, WalletCards,
} from "lucide-react";
...
export default function StaffShell({ children }: StaffShellProps) {
  ...
  const sidebarPreferenceKey = `posq.sidebar.expanded.${user?.id ?? "default"}`;
  const visibleGroupsExpanded = visibleNavigation.length > 0 && visibleNavigation.every(group => expandedGroups[group.group]);

  useEffect(() => {
    if (!accessToken) return;
    const saved = window.localStorage.getItem(sidebarPreferenceKey);
    setExpandedGroups(saved === "true" ? allGroupsExpanded : allGroupsCollapsed);
  }, [accessToken, sidebarPreferenceKey, allGroupsCollapsed, allGroupsExpanded]);
  ...
  const toggleAllGroups = () => {
    const nextExpanded = !visibleGroupsExpanded;
    setExpandedGroups(nextExpanded ? allGroupsExpanded : allGroupsCollapsed);
    window.localStorage.setItem(sidebarPreferenceKey, String(nextExpanded));
  };
  ...
  return (
    <div className="min-h-screen bg-[#f5faf8] text-[#173f3d]">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[252px] flex-col bg-[#103d3b] px-4 py-5 text-[#f7fbf7] shadow-2xl transition-transform lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <button type="button" onClick={toggleAllGroups} aria-label={visibleGroupsExpanded ? "Collapse all menu groups" : "Expand all menu groups"} className="mb-3 flex w-full items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-[#c9d7d0] transition hover:bg-white/10 hover:text-white">
          <span>{visibleGroupsExpanded ? "Collapse all menus" : "Expand all menus"}</span><ChevronsDownUp className="h-4 w-4" />
        </button>
