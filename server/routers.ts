...
export const appRouter = router({
  ...
  hris: router({
    ...
    selfService: router({
      profile: staffProcedure.query(async ({ ctx }) => (await getEmployeeProfileByUserId(ctx.staff.userId)) ?? null),
      leave: router({
  ...
  timekeeping: router({
    ...
    calendar: staffProcedure.input(z.object({ locationId: z.number().int().positive(), start: z.coerce.date(), end: z.coerce.date() })).query(async ({ ctx, input }) => { await requireLocationAccess(ctx, input.locationId); const rows = await listStaffAttendanceRange(input.locationId, input.start, input.end); return ctx.staff.role === "cashier" ? rows.filter(row => row.userId === ctx.staff.userId) : rows; }),
    ...
    payroll: router({
      runs: managerProcedure.query(() => listPayrollRuns()),
      detail: managerProcedure.input(z.object({ id: z.number().int().positive() })).query(({ input }) => getPayrollRun(input.id)),
      compute: managerProcedure.input(z.object({ periodStart: z.coerce.date(), periodEnd: z.coerce.date() })).mutation(async ({ ctx, input }) => { if (input.periodEnd <= input.periodStart) throw new TRPCError({ code: "BAD_REQUEST", message: "Payroll period end must be after period start" }); const result = await computePayroll({ ...input, computedById: ctx.staff.userId }); await appendAuditLog({ userId: ctx.staff.userId, action: "payroll.computed", entityType: "payroll_run", entityId: result.runId, metadata: { periodStart: input.periodStart.toISOString(), periodEnd: input.periodEnd.toISOString(), itemCount: result.itemCount } }); return result; }),
      updateStatus: adminStaffProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["approved", "paid"]) })).mutation(async ({ ctx, input }) => { await updatePayrollRunStatus(input.id, input.status, ctx.staff.userId); await appendAuditLog({ userId: ctx.staff.userId, action: `payroll.${input.status}`, entityType: "payroll_run", entityId: input.id }); return { success: true }; }),
    }),
    schedules: router({
  ...
  procurement: router({
    requests: managerProcedure.input(z.object({ locationId: z.number().int().positive() })).query(async ({ ctx, input }) => { await requireLocationAccess(ctx, input.locationId); return listPurchaseRequests(input.locationId); }),
    requestDetail: managerProcedure.input(z.object({ requestId: z.number().int().positive() })).query(async ({ input }) => { const result = await getPurchaseRequest(input.requestId); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Purchase request was not found" }); return result; }),
    createRequest: managerProcedure.input(z.object({ locationId: z.number().int().positive(), note: z.string().trim().max(1000).optional(), items: z.array(z.object({ productId: z.number().int().positive(), quantityRequested: quantitySchema.refine(value => Number(value) > 0), note: z.string().trim().max(500).optional() })).min(1) })).mutation(async ({ ctx, input }) => { await requireLocationAccess(ctx, input.locationId); const requestId = await createPurchaseRequest({ ...input, requestNumber: `PR-${Date.now()}-${ctx.staff.userId}`, requestedById: ctx.staff.userId }); await appendAuditLog({ userId: ctx.staff.userId, locationId: input.locationId, action: "purchase_request.created", entityType: "purchase_request", entityId: requestId, metadata: { lineCount: input.items.length } }); return { requestId }; }),
    updateRequest: managerProcedure.input(z.object({ requestId: z.number().int().positive(), status: z.enum(["submitted", "approved", "rejected"]), rejectionReason: z.string().trim().max(500).optional() })).mutation(async ({ ctx, input }) => { const request = await getPurchaseRequest(input.requestId); if (!request) throw new TRPCError({ code: "NOT_FOUND", message: "Purchase request was not found" }); await requireLocationAccess(ctx, request.locationId); await updatePurchaseRequestStatus(input.requestId, input.status, ctx.staff.userId, input.rejectionReason); await appendAuditLog({ userId: ctx.staff.userId, locationId: request.locationId, action: `purchase_request.${input.status}`, entityType: "purchase_request", entityId: input.requestId }); return { success: true }; }),
    orders: managerProcedure.input(z.object({ locationId: z.number().int().positive() })).query(async ({ ctx, input }) => { await requireLocationAccess(ctx, input.locationId); return listPurchaseOrders(input.locationId); }),
    orderDetail: managerProcedure.input(z.object({ orderId: z.number().int().positive() })).query(async ({ input }) => { const result = await getPurchaseOrder(input.orderId); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Purchase order was not found" }); return result; }),
    createOrder: managerProcedure.input(z.object({ locationId: z.number().int().positive(), requestId: z.number().int().positive().optional(), supplierName: z.string().trim().min(2).max(180), note: z.string().trim().max(1000).optional(), items: z.array(z.object({ productId: z.number().int().positive(), quantityOrdered: quantitySchema.refine(value => Number(value) > 0), unitCost: phpAmountSchema })).min(1) })).mutation(async ({ ctx, input }) => { await requireLocationAccess(ctx, input.locationId); const orderId = await createPurchaseOrder({ ...input, orderNumber: `PO-${Date.now()}-${ctx.staff.userId}`, createdById: ctx.staff.userId }); await appendAuditLog({ userId: ctx.staff.userId, locationId: input.locationId, action: "purchase_order.created", entityType: "purchase_order", entityId: orderId, metadata: { supplierName: input.supplierName } }); return { orderId }; }),
    updateOrder: managerProcedure.input(z.object({ orderId: z.number().int().positive(), status: z.enum(["submitted", "approved", "ordered", "cancelled"]) })).mutation(async ({ ctx, input }) => { const order = await getPurchaseOrder(input.orderId); if (!order) throw new TRPCError({ code: "NOT_FOUND", message: "Purchase order was not found" }); await requireLocationAccess(ctx, order.locationId); await updatePurchaseOrderStatus(input.orderId, input.status, ctx.staff.userId); await appendAuditLog({ userId: ctx.staff.userId, locationId: order.locationId, action: `purchase_order.${input.status}`, entityType: "purchase_order", entityId: input.orderId }); return { success: true }; }),
    receiveOrder: managerProcedure.input(z.object({ orderId: z.number().int().positive(), items: z.array(z.object({ itemId: z.number().int().positive(), quantityReceived: quantitySchema.refine(value => Number(value) > 0) })).min(1) })).mutation(async ({ ctx, input }) => { const order = await getPurchaseOrder(input.orderId); if (!order) throw new TRPCError({ code: "NOT_FOUND", message: "Purchase order was not found" }); await requireLocationAccess(ctx, order.locationId); const result = await receivePurchaseOrder(input.orderId, ctx.staff.userId, input.items); await appendAuditLog({ userId: ctx.staff.userId, locationId: order.locationId, action: "purchase_order.received", entityType: "purchase_order", entityId: input.orderId, metadata: { complete: result.complete } }); return result; }),
  }),
  loyalty: router({
