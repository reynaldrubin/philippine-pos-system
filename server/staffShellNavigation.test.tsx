...
describe("StaffShell protected navigation", () => {
  ...
  it("shows Reports to a manager with assigned Reports access and routes to the workspace", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Reports submenu" }));
  ...
  it("starts authenticated navigation groups collapsed without a global toggle", () => {
    render(<StaffShell><div>Protected workspace</div></StaffShell>);
    expect(screen.queryByRole("button", { name: "Expand all menu groups" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Reports" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Workspace submenu" }));
