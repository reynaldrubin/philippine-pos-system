...
export const payrollRuns = mysqlTable("payrollRuns", {
  id: int("id").autoincrement().primaryKey(), periodStart: timestamp("periodStart").notNull(), periodEnd: timestamp("periodEnd").notNull(), status: mysqlEnum("status", ["computed", "approved", "paid"]).default("computed").notNull(), totalGross: decimal("totalGross", { precision: 14, scale: 2 }).default("0").notNull(), totalDeductions: decimal("totalDeductions", { precision: 14, scale: 2 }).default("0").notNull(), totalNet: decimal("totalNet", { precision: 14, scale: 2 }).default("0").notNull(), computedById: int("computedById").notNull().references(() => users.id, { onDelete: "restrict" }), approvedById: int("approvedById").references(() => users.id, { onDelete: "set null" }), approvedAt: timestamp("approvedAt"), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [index("payroll_runs_period_idx").on(table.periodStart, table.periodEnd), index("payroll_runs_status_idx").on(table.status)]);

export const payrollItems = mysqlTable("payrollItems", {
  id: int("id").autoincrement().primaryKey(), payrollRunId: int("payrollRunId").notNull().references(() => payrollRuns.id, { onDelete: "cascade" }), employeeId: int("employeeId").references(() => employeeProfiles.id, { onDelete: "set null" }), userId: int("userId").notNull().references(() => users.id, { onDelete: "restrict" }), daysWorked: decimal("daysWorked", { precision: 8, scale: 2 }).default("0").notNull(), hoursWorked: decimal("hoursWorked", { precision: 8, scale: 2 }).default("0").notNull(), lateMinutes: int("lateMinutes").default(0).notNull(), undertimeMinutes: int("undertimeMinutes").default(0).notNull(), overtimeMinutes: int("overtimeMinutes").default(0).notNull(), basePay: decimal("basePay", { precision: 14, scale: 2 }).default("0").notNull(), overtimePay: decimal("overtimePay", { precision: 14, scale: 2 }).default("0").notNull(), allowances: decimal("allowances", { precision: 14, scale: 2 }).default("0").notNull(), deductions: decimal("deductions", { precision: 14, scale: 2 }).default("0").notNull(), netPay: decimal("netPay", { precision: 14, scale: 2 }).default("0").notNull(), notes: text("notes"), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [index("payroll_items_run_idx").on(table.payrollRunId), index("payroll_items_user_idx").on(table.userId)]);
